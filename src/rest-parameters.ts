 ...
 * @returns {number} 足し算の算術結果（1つの値）
 */

const reducer = (acc: number, cur: number) => {
  console.log({ acc:acc, cur:cur }); // 省略記法（プロパティ名と値に指定する変数名が同じ場合は { name } のように省略して書ける）
  return acc + cur; // 関数の返り値は「アキュムレーター」に代入される
}
